...
class Web3Client:
    def __init__(self, client, network, logger, config):
    ...
    def get_balance(self):
        return self.web3.eth.get_balance(account=self.address)
    ...
    def get_address_from_private_key(self, private_key):
        return self.web3.to_checksum_address(self.web3.eth.account.from_key(private_key=private_key).address)
    ...
    def prepare_transaction(self, value=0):
        ...
        if self.network.eip1559_support:
            ...
            max_fee_per_gas = int(base_fee + max_priority_fee_per_gas * 1.05 * self.config.gas_price_multiplier)
            ...
            tx_params['gasPrice'] = int(gas_price * 1.2 * self.config.gas_price_multiplier)

        return tx_params

    def transaction_runner(self, transaction):
        transaction['gas'] = int((self.web3.eth.estimate_gas(transaction)) * self.config.gas_limit_multiplier)
    ...
    def send_transaction(self, transaction):
        ...
        if self.get_transaction(submitted_transaction=submitted_transaction).status:
            ...
            raise Exception(f'[{self.address}] | transaction failed 📕')
    ...
    def get_balanceOf(self, contract, validate=None):
        if validate:
            decimals = int(contract.functions.decimals().call())
            if decimals != 18:
                missing_decimals = 18 - int(decimals)
            else:
                missing_decimals = 0

            return int(str(contract.functions.balanceOf(
                self.web3.to_checksum_address(self.address),
            ).call()) + missing_decimals * '0')
        else:
            return int(str(contract.functions.balanceOf(
                self.web3.to_checksum_address(self.address),
            ).call()))
    ...
    def check_for_approved(
        ...
        balance_in_wei = self.get_balanceOf(contract=contract)
        ...
        if balance_in_wei <= approved_amount_in_wei:
            return False

        self.make_approve(token_address, spender_address, amount_in_wei, unlimited_approve, symbol)
